...
// This part is on 8 bit interface
	LPC_GPIO0->FIOSET = DB5_PIN; // DB7=0, DB6=0, DB5=1, DB4=1,
	LPC_GPIO0->FIOSET = DB4_PIN; // NU=0, EN=0, RW=0, RS=0
	delayUs(1);		 // Wait a minimum of 60ns
		//repeat (function set) 3 times 
	LPC_GPIO0->FIOSET = EN;		 // Raise EN start write operation
	delayUs(1);		 // Wait a minimum of 195ns
	LPC_GPIO0->FIOCLR = EN;		 // Clear EN finish write operation
...
// This part is on 8 bit interface
	...
	LPC_GPIO0->FIOSET = DB5_PIN; // DB7=0, DB6=0, DB5=1, DB4=0, NU=0, EN=0, RW=0, RS=0
	LPC_GPIO0->FIOCLR = DB4_PIN; // SETS 4-bit operation
	delayUs(1);		 // Wait a minimum of 60ns
...
unsigned char WriteByte(unsigned char rs, unsigned char data_to_LCD)
{
	...
	if(rs)											
		LPC_GPIO0->FIOSET = RS;								// Set RS (DR operation)
	else
		LPC_GPIO0->FIOCLR = RS;								// Clear RS (IR operation)

	delayUs(1);									// Wait a minimum of 60ns
	
	LPC_GPIO0->FIOSET = EN;									// Set EN in order to start first write operation
	...
	LPC_GPIO0->FIOPIN = (LPC_GPIO0->FIOPIN & ~DATA_BUS)|dataVal;		// data bus reflects upper nibble
	delayUs(1);									// Wait a minimum of 195ns

	LPC_GPIO0->FIOCLR = EN;									// Clear EN finish first write operation
	delayUs(1);									// Wait a minimum of 530ns between writes

	LPC_GPIO0->FIOSET = EN;									// Raise EN start second write operation

	dataVal = data_to_LCD & 0x0F;				// Copy lower nibble into dataVal
	dataVal = dataVal << DB4_PIN_SHIFT;			// shift left on dataVal to align DB4
	LPC_GPIO0->FIOPIN = (LPC_GPIO0->FIOPIN & ~DATA_BUS)|dataVal;		// data bus reflects lower nibble
	delayUs(1);									// Wait a minimum of 195ns

	LPC_GPIO0->FIOCLR = EN;									// Clear EN finish second write operation
